...
@Named("studentProfileGenerator")
@RequestScoped
public class StudentProfileExcelGenerator implements Serializable {
	...
	//Student Profile
    public void createExcel(List<StudentProfile> studentProfileList){
		
    	
		System.out.println("STUDENT LIST: "+studentProfileList);
		HSSFWorkbook studentbook = new HSSFWorkbook(); 
		  
		HSSFSheet sheet = studentbook.createSheet("Student Profile Data");
		
		    Map<Integer, Object[]> data = new TreeMap<Integer, Object[]>();

	        data.put(0, new Object[] {"PROFILE ID", "GENDER", "BIRTHDAY", "CONTACT NO", "EMAIL", "YEAR LEVEL", "SECTION"});
	        
	        for(int i = 0; i < studentProfileList.size(); i++){
	        data.put(i+1, new Object[] {studentProfileList.get(i).getProfileId(), studentProfileList.get(i).getGender(), studentProfileList.get(i).getBirthDay(), studentProfileList.get(i).getContactNo(), studentProfileList.get(i).getEmailAdd(), studentProfileList.get(i).getYearLevel(), studentProfileList.get(i).getSection()});
	        }
	        ...
	        try
	        {
	            ...
	            FileOutputStream out = new FileOutputStream(new File("/home/lorenzor/studentprofile.xlsx"));
